<?php
    ...
    require_once('../Services/DNA/DNAService.php');
    
    // Get DNA data
    $app->get('/dna', $referer($app), $authorization($app), function() use ($app)
    {
        $log = $app->log;
        $db = $app->db;
        $dbw = $app->dbw;
        $scope = $app->jwt->scope;
        $access = GetAccess($scope, $app->pages['sites']['dna'], $db, $log);
        $data = GetDNA($db, $log, $dbw);
        echo '{"access":"' . $access .'", "data":' .$data .'}';

    });

    //GET info about dna
    $app->get('/dna/:id', $referer($app), $authorization($app), function($id) use ($app)
    {   
        $log = $app->log;
        $db = $app->db;
        $dbw = $app->dbw;
