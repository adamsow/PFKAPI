...
function SaveExhibitionData($db, $log, $body)
{
	...
	$stmt->bindParam(':kennel1', $data->stud5);
	$stmt->bindParam(':kennel2', $data->bitch4);
	$stmt->bindParam(':kennel3', $data->bitch5);
	$stmt->bindParam(':kennel4', $data->bitch6);
	$stmt->bindParam(':kennel5', $data->pair1);
	$stmt->bindParam(':kennel6', $data->pair2);
...
function alreadyExists($db, $data)
{
	$stmt = $db->prepare("SELECT count(*) FROM ".$data->exhibition." 
