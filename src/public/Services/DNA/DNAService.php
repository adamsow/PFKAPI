...
require_once('ValidateDNAService.php');

function GetDNA($db, $log, $dbw)
{
	$log -> addInfo("Getting DNA data.");
	$stmt = $db->prepare("SELECT id as Id, fullname, owner, sampling_date, 
                        DNA_member_id as prober
						FROM DNA;");
	
	$stmt->execute();
    $DNA = $stmt->fetchAll();
    foreach ($DNA as $key => $DNA_single) 
    {
        $prober = GetUser($DNA_single['prober'], $dbw);
